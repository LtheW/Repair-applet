...
Page({
  ...
  Load: function () {
    var app = getApp();
    var that = this;
    console.log(that.data.workorderNumber)
    wx.request({
      data: {
        workorder_number: that.data.workorderNumber
      },
      url: app.globalData.url + 'student/getorder',
      success: function (res) {
        console.log(res);
        var jsonStr = res.data.data[0].pictureAddress;
        ...
        that.setData({
          workOrderState: parseInt(res.data.data[0].workorderState) - 1,
  ...
  evaluation:function () {
    ...
    console.log(that.data.workorderNumber);
    wx.navigateTo({
      url: '../evaluation/evaluation?workorderNumber='+that.data.workorderNumber,
    })
  },
  /**
   * 生命周期函数--监听页面加载
